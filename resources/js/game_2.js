...
GamePlayState.prototype.update = function(d_t)
{
    ...
    for( i = 0; i < this.active_coins.length; i++)
    {
        var active_coin = this.active_coins[i]; 
        active_coin.updateObj(d_t);
        
        if( ! active_coin.visable )
        {
            //To do. Remove coins from the active_array and the "drive" scene
        }
    }
    ...
    if( this.c_disp_dist_left <= 0 )
    {
        ...
        coin.setState(new CoinState_Active(this.game,coin));
